...
Перем ПутьКВременномуФайлуИзменений;
...
//
Процедура ПередНачаломВыполнения(ПутьКХранилищу, КаталогРабочейКопии) Экспорт // BSLLS:UnusedParameters-off
...
Процедура ПриВыгрузкеКонфигурациюВИсходники(Конфигуратор, КаталогВыгрузки, СтандартнаяОбработка) Экспорт
	...
	Если ВыгрузкаИзмененийВозможна Тогда
		...
		Если ФС.ФайлСуществует(ПутьКВременномуФайлуИзменений) Тогда
			ПереместитьФайл(ПутьКВременномуФайлуИзменений, ОбъединитьПути(КаталогВыгрузки, ИмяФайлаИзменений));
		КонецЕсли;
...
//
// Параметры:
//   Конфигуратор - Конфигуратор - объект конфигуратора
//   ПутьКФайлуВерсийДляСравнения - Строка - путь к файлу версий для сравнения
//
//  Возвращаемое значение:
...
Функция ПроверитьВозможностьОбновленияФайловВыгрузки(Конфигуратор, Знач ПутьКФайлуВерсийДляСравнения = "")
	...
	ПутьКВременномуФайлуИзменений = ВременныеФайлы.НовоеИмяФайла();

	Параметры = Конфигуратор.ПолучитьПараметрыЗапуска();
	Параметры.Добавить(СтрШаблон("/DumpConfigToFiles %1", ОбернутьВКавычки(КаталогПроверки)));
	Параметры.Добавить(СтрШаблон("-getChanges %1", ОбернутьВКавычки(ПутьКВременномуФайлуИзменений)));
	...
	Если ФайлИзменений.Существует() Тогда
		...
		ЧтениеФайла = Новый ЧтениеТекста(ПутьКВременномуФайлуИзменений);
	...
	Если НЕ ОбновлениеВозможно Тогда
		ВременныеФайлы.УдалитьФайл(ПутьКВременномуФайлуИзменений);
	КонецЕсли;
