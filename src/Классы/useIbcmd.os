...
Перем ТекущаяКоманда;   //   Строка              - имя выполняемой команды gitsync // BSLLS:UnusedLocalVariable-off
...
Перем ПутьКФайлуПроверки;               //   Строка    - путь к файлу проверки
Перем ПутьКФайлуИзменений;              //   Строка    - путь к файлу с информацией об изменениях в конфигурации
...
// BSLLS:UnusedParameters-on
	...
	ИнкрементальнаяВыгрузкаВозможна = (Инкрементально
	                                 И ИнкрементальнаяВыгрузкаВозможна(Конфигуратор));
...
//
Функция ИнкрементальнаяВыгрузкаВозможна(Знач Конфигуратор) // BSLLS:UnusedParameters-off
	...
	Если НЕ ФайлДампаИзменений.Существует() Тогда
		...
		Лог.Информация("Тип выгрузки конфигурации в файлы: ПОЛНАЯ ВЫГРУЗКА");
		Возврат Результат;
	КонецЕсли;
	...
	ВыводКоманды = ВыполнитьКомандуIBCMD(Конфигуратор);

	Результат = ИнкрементальнаяВыгрузкаВозможнаПоВыводуIBCMD(ВыводКоманды);
	
	СпособВыгрузки = ?(Результат, "ИНКРЕМЕНТАЛЬНАЯ ВЫГРУЗКА", "ПОЛНАЯ ВЫГРУЗКА");
	Лог.Информация("Тип выгрузки конфигурации в файлы: %1%2", СпособВыгрузки, Символы.ПС);

	Возврат Результат;

КонецФункции // ИнкрементальнаяВыгрузкаВозможна()

Функция ВыполнитьКомандуIBCMD(Конфигуратор)

	ПутьКФайлуПроверки = ПолучитьИмяВременногоФайла("dmp"); // BSLLS:MissingTemporaryFileDeletion-off
	...
	Если КодВозврата = 0 Тогда
		...
		Если ФайлПроверки.Существует() Тогда
			...
			ВыводКоманды = ЧтениеФайла.ПрочитатьСтроку();
			ЧтениеФайла.Закрыть();
...
Функция ИнкрементальнаяВыгрузкаВозможнаПоВыводуIBCMD(ВыводКоманды)

	Результат = "";

	СтрокаПолныйДамп = "modified: all";

	СтрокаПроверки = Лев(ВыводКоманды, СтрДлина(СтрокаПолныйДамп));
