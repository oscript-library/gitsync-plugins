...
Функция ВыгрузитьТаблицыВXML(Знач ИменаТаблиц) Экспорт
	...
	СтрокаЗапуска = """"+ ПрефиксПути + мИмяФайлаБазы + """"
			+ " -l """ +ПрефиксПути+ ЛогTool1CD + """"
			+ " -q -ne -ex " 
			+ """" + ПрефиксПути + РабочийКаталог() + """ """  + ИменаТаблиц + """";
			
	ПутьПриложения = ПутьTool1CD();
	Если Не ЭтоWindows Тогда 
		СтрокаЗапуска = СтрШаблон("""%1"" %2", ПутьПриложения, СтрокаЗапуска);
		ПутьПриложения = "wine";
	КонецЕсли;

	Лог.Отладка(ПутьПриложения);
	Лог.Отладка(СтрокаЗапуска);
	
	Команда = Новый Команда;
	Команда.УстановитьКоманду(ПутьПриложения);
	Команда.УстановитьПараметры(СтрокаЗапуска);
