...
Процедура ПриСозданииОбъекта()
	...
	ЭтоСборкаEXE = ВРег(Прав(ТекущийСценарий().Источник, 3)) = "EXE"; // BSLLS:MagicNumber-off
...
Процедура ПодготовитьКомпоненту()
	...
	Иначе
		...
		МажорнаяВерсия = СтрРазделить(СИ.Версия, ".")[0];
		МинорнаяВерсия = Число(СтрРазделить(СИ.Версия, ".")[1]);

		Если МажорнаяВерсия = "1" Тогда
			Если МинорнаяВерсия = 0 Тогда
				ИмяФайлаМакета = "v8unpack_1.0.19.dll";
			Иначе
				ИмяФайлаМакета = "v8unpack_1.9.dll";
			КонецЕсли;
		Иначе
			ИмяФайлаМакета = "v8unpack.dll";
		КонецЕсли;

		ПутьКФайлу = ЗагрузчикДвоичныхДанных.ПолучитьПутьКФайлу(ИмяФайлаМакета);
