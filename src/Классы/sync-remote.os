...
Процедура ПриРегистрацииКомандыПриложения(ИмяКоманды, КлассРеализации) Экспорт
	...
	КлассРеализации.Опция("b branch", "master", "[*sync-remote] <Имя ветки git>").ВОкружении("GITSYNC_BRANCH");
...
Процедура ПриПолученииПараметров(ПараметрыКоманды) Экспорт
	...
	Лог.Отладка("Установлен URL репозитория <%1> ", URLРепозитория);
	Лог.Отладка("Установлена ветка репозитория <%1> ", ИмяВетки);
...
Процедура ВыполнитьGitPush(Знач ГитРепозиторий, Знач ЛокальныйРепозиторий)
	...
	Если ПустаяСтрока(ИмяВетки) Тогда
	...
	Иначе
		ГитРепозиторий.Отправить(URLРепозитория, ИмяВетки);
	КонецЕсли;
