...
Процедура ПриРегистрацииКомандыПриложения(ИмяКоманды, КлассРеализации) Экспорт
	...
	КлассРеализации.Опция("SP sort-project", Ложь, "[*edtExport] автосортировка проекта")
				   .ТБулево()
	               .ВОкружении("GITSYNC_SORT_PROJECT");
...
Процедура ПриПолученииПараметров(ПараметрыКоманды) Экспорт
	...
	АвтосортировкаПроекта = ПараметрыКоманды.Параметр("sort-project", Ложь);
...
// BSLLS:UnusedParameters-on
	...
	КаталогSettingsОткуда = ОбъединитьПути(КаталогРабочейКопии, ".settings");
	КаталогSettingsКуда = ОбъединитьПути(КаталогПроекта, ".settings");
	ФС.КопироватьСодержимоеКаталога(КаталогSettingsОткуда, КаталогSettingsКуда);
	...
	Если Использовать1cedtcli И АвтосортировкаПроекта Тогда
		Лог.Отладка("Запускаем сортировку объектов");
		Команда = КомандаСортировкиОбъектов(КаталогПроекта, ВременноеРабочееПространство);
		КодВозврата = Команда.Исполнить();
		Лог.Отладка("Код возврата команды сортировки: %1", КодВозврата);
	КонецЕсли;
...
Процедура ПриОчисткеКаталогаРабочейКопии(КаталогРабочейКопии,               // BSLLS:UnusedParameters-off
                                         СоответствиеИменФайловДляПропуска, // BSLLS:UnusedParameters-off
                                         СтандартнаяОбработка) Экспорт

	СоответствиеИменФайловДляПропуска.Вставить(".settings", Истина);
...
// Функция - получает команду сортировки объектов проекта утилиты 1cedtcli
//
// Параметры:
//   КаталогПроекта - Строка - Путь к каталогу проекта, объекты которого нужно сортировать.
//   ВременноеРабочееПространство - Строка - Временная рабочая область проекта.
//
// Возвращаемое значение:
//  Команда - команда сортировки sort-project утилиты 1cedtcli
//
Функция КомандаСортировкиОбъектов(КаталогПроекта, ВременноеРабочееПространство)
	
	Команда = Новый Команда;
	Команда.УстановитьКоманду(ОпределитьИсполняемыйФайл());
	
	Команда.ДобавитьПараметр("-data");
	Команда.ДобавитьПараметр(ОбернутьВКавычки(ВременноеРабочееПространство));
	Команда.ДобавитьПараметр("-command sort-project");
	
	Команда.ДобавитьПараметр(СтрШаблон("--project-list ""%1""", КаталогПроекта));
