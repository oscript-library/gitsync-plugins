...
Сценарий: Cинхронизация с использованием push
    Допустим Я добавляю параметр "--PS" для команды "gitsync"
...
Сценарий: Cинхронизация с использованием pull и push
    Допустим Я добавляю параметр "--PS" для команды "gitsync"
    И Я добавляю параметр "-G" для команды "gitsync"
    И Я добавляю позиционный параметр для команды "gitsync" из переменной "КаталогХранилища1С"
    И Я добавляю позиционный параметр для команды "gitsync" из переменной "ПутьКаталогаИсходников"
    И Я добавляю позиционный параметр для команды "gitsync" из переменной "URLРепозитория"
    И Я записываю "9" в файл VERSION
...
Сценарий: Cинхронизация хранилища все вместе
    Допустим Я добавляю параметр "--PS" для команды "gitsync"
    И Я добавляю параметр "-GTn=2" для команды "gitsync"
    И Я добавляю позиционный параметр для команды "gitsync" из переменной "КаталогХранилища1С"
    И Я добавляю позиционный параметр для команды "gitsync" из переменной "ПутьКаталогаИсходников"
    И Я добавляю позиционный параметр для команды "gitsync" из переменной "URLРепозитория"
    И Я записываю "6" в файл VERSION
    Когда Я выполняю команду "gitsync"
    Тогда Вывод команды "gitsync" содержит "Завершена синхронизации с git"
    И Вывод команды "gitsync" не содержит "Внешнее исключение"
    И Код возврата команды "gitsync" равен 0
