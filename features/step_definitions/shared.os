...
//Я устанавливаю рабочий каталог из переменной "ПутьКаталогаИсходников"
Процедура ЯУстанавливаюРабочийКаталогИзПеременной(Знач ПутьКаталогаИсходников) Экспорт
...
Процедура ЯИнициализируюРепозиторийВКаталогеИзПеременной(Знач ПутьКаталогаИсходников) Экспорт
	...
	ГитРепозиторий.УстановитьНастройку("user.name", "John Doe", РежимУстановкиНастроекGit.Локально);
	ГитРепозиторий.УстановитьНастройку("user.email", "jd@example.com", РежимУстановкиНастроекGit.Локально);
...
Процедура ЯУстанавливаюТекущиеПлагины() Экспорт
	...
	КомандаOpm.ДобавитьПараметр("run install-plugins");
...
Функция ПутьКГитсинк()
	...
	Возврат ОбъединитьПути(КореньПроекта(), "bin_gitsync", "gitsync/src/cmd/gitsync.os");
